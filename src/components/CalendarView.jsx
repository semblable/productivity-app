...
const EventComponent = ({ event }) => {
    ...
    const isSynced = !!event.resource.gcalEventId;
    return (
        <div className="flex flex-col p-1">
            <div className="flex items-center gap-1">
                <strong className="font-semibold">{event.title}</strong>
                {isSynced && (
                    <span title="Synced with Google Calendar" className="opacity-70 text-xs">🔗</span>
                )}
            </div>
...
export const CalendarView = ({ date, view, onNavigate, onView, onSelectSlot, onSelectEvent }) => {
    ...
    const formattedEvents = (events || [])
        .flatMap((event) => {
            ...
            if (event.rrule && !event.templateId && !event.parentId) {
                try {
                    ...
                    if (!ruleStr.includes('DTSTART')) {
                        ...
                        const formattedLines = lines.map(l => {
                            if (l.startsWith('RRULE:') || l.startsWith('EXDATE:') || l.startsWith('RDATE:') || l.startsWith('EXRULE:')) {
                                return l;
                            }
                            return `RRULE:${l}`;
                        });
                    ...
                    const rruleObj = rrulestr(ruleStr, { forceset: true });
                    const windowStart = subYears(new Date(), 1);
                    const windowEnd = addYears(new Date(), 2);
                    
                    const instances = rruleObj.between(windowStart, windowEnd, true);
