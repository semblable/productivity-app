...
export const AddEventModal = ({ isOpen, onClose, eventData, projects, onStartTracking }) => {
    ...
    const executeSave = async (updateScope) => {
        ...
        try {
            ...
            api.gcal.sync().catch(console.error); // Trigger instant sync to Google
            onClose();
        } catch (error) {
    ...
    const executeDelete = async (deleteScope) => {
        ...
        try {
            let parentEvent = eventData;
            if (eventData.parentId) {
                try {
                    parentEvent = await api.events.get(eventData.parentId);
                } catch (err) {
                    console.warn("Parent event missing. Falling back to simple delete.");
                    parentEvent = null;
                }
            }
            ...
            if (!parentEvent) {
                ...
                toast.success("Orphaned event deleted.");
